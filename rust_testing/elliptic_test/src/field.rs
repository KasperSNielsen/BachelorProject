...
pub fn fp2mul(n: Fp2, m: Fp2) -> Fp2 { //Using u^2 = -1
    let (n1, n2) = n;
    let (m1, m2) = m;
    let x1 = n1 * m1 - (n2 * m2);
    let x2 = n1 * m2 + n2 * m1;
...
pub fn fieldfis() -> G1 {
    let zero = Fp::ZERO(); //Create element Zero from public_nat_mod macro called Fp
    let two = Fp::TWO(); // Same as above
    let maxval = Fp::max_val(); //Create max value, add 2, which would equal 1 under the mod
...
{
    let mut resultpoint = (Fp::ZERO(), Fp::ZERO(), true); //G1 at infinity
...
pub fn double(p: G1, a: Fp) -> G1
{
    ...
    if !inf && y_1 != Fp::ZERO()
    {
        ...
        let xovery = (Fp::from_literal(3u128) * x_12 + a) * (Fp::TWO() * y_1).inv();
        let x_3 = xovery.exp(2u32) - Fp::TWO()*x_1;
        let y_3 = xovery * (x_1 - x_3) - y_1;
        resultpoint = (x_3, y_3, false);    
    }
...
pub fn mult(m: Scalar, p: G1, a: Fp) -> G1
{
    let n = 6;
